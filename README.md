...
* **Now, showing CPU usage of For and While Loop by using **"psutill"(Python system and process utilities)** module, which fetch cpu usage details from API's which are being use by any operating system . ex- (cat /proc/stat)- we can use this command for checking CPU usage in Ubuntu. psutill module fetch data from /proc/stat and give usage details for execution of loops.**
